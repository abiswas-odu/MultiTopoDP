...
class MRC
{
	private: int nx;//number of columns (fastest changing in map)
		 ...
		 int nxStart;//number of first column in map (Defalut = 0)
	 	 ...
	 	 float ***cube;//matrix points of density map
	public:
		 MRC()//consturctor
		 {
		 }
		 ...
		 void output(std::string file);//output the lattice and value
		 void EMRead(std::string file);//read density map by EMan formate
		 void read(std::string file);//read density map from binary file
		 void save(std::string file);//save density map to binary file

		 MRC operator=(MRC &right);//get a copy of right

		 void pdb(ofstream &file);//save voxel to pdb file
		 void pdb(std::string);//save voxel to pdb file

		 void show();//show density map information
